...
int main(int argc, char **argv)
{
    ...
    PrefetchRWMHChain::h = argc>2 ? atol(argv[2]) : 1;

    int par = argc>3 ? atol(argv[3]) : 1;
    ...
    switch (par) {
        ...
        case 1: 
            ...
            std::clog << "PrefetchRWMHChain" << std::endl;
            break;
        case 2: 
            MyMH = new PrefetchRWMHChainOMP;
            std::clog << "PrefetchRWMHChainOMP" << std::endl;
            /*{
                double x = 0.0;
        ...
        case 3: 
            ...
            std::clog << "PrefetchRWMHChainCilk" << std::endl;
            /*{
                double x = 0.0;
                #pragma omp parallel for reduction (+:x)
                for(int i=0; i<10000; ++i)
    ...
    if (par > 0) {
        ...
        for(int i=0; i<n; ++i) {
            int s = dynamic_cast<PrefetchRWMHChain*>(MyMH)->pref_at_step[i];
            if(s > 0) {
                sum_s += s;
                count_s += 1.0;
                if( s > max_s ) max_s = s;
