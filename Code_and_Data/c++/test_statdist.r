...
sqv <- sqrt(vars)
ret <- .C("mvnm_rand", w=as.integer(0), n=as.integer(n), vals=as.double(rep(0,2*n)),
            k=as.integer(length(pr)), as.double(pr), as.integer(length(mu)/length(pr)),
            as.double(mu), as.double(vars), DUP=FALSE)
...
ret <- .C("mvnm_pdf", n=as.integer(n), as.double(QQ), p=as.double(rep(0,n)),
            as.integer(length(pr)), as.double(pr), as.integer(length(mu)/length(pr)),
            as.double(mu), as.double(vars), DUP=FALSE)
